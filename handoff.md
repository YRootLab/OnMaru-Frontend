...
- Vercel direct deployment는 `vercel pull`을 쓰지 않고, CI에서 `.vercel/project.json`에 `orgId`/`projectId`와 빈 `settings`를 써 넣은 뒤 `vercel build` + `vercel deploy --prebuilt`를 수행한다. 현재 토큰은 project API 조회는 가능하지만 `pull`의 project settings 조회와 `--scope="$VERCEL_ORG_ID"` 둘 다 실패하므로, private Core UI submodule checkout과 `npm run check:submodule` 검증을 유지한 채 prebuilt 배포 경로를 복구한다.
