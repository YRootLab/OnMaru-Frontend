...
- Vercel direct deployment는 `vercel pull/build --prebuilt` 경로 대신 직접 `vercel deploy`를 사용한다. 현재 토큰은 project API 조회는 가능하지만 `pull`의 project settings 조회와 `--scope="$VERCEL_ORG_ID"` 둘 다 실패하므로, private Core UI submodule checkout은 유지한 채 direct deploy로 우회한다.
