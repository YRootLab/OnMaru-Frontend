...
- Vercel direct deployment를 `pull -> build -> deploy --prebuilt`에서 직접 `vercel deploy`로 단순화해, 현재 토큰이 `Project Settings` 조회와 team `--scope`를 처리하지 못해 발생하던 Actions 배포 실패를 우회했다.
