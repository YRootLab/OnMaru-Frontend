...
- Vercel direct deployment는 `vercel pull`을 제거하되, CI에서 `.vercel/project.json`에 Vercel project metadata와 빈 `settings`를 기록한 뒤 `vercel build` + `vercel deploy --prebuilt`를 실행하도록 바꿔 현재 토큰의 `Project Settings`/team `--scope` 제약을 우회하면서 기존 prebuilt 배포 semantics를 유지했다.
