...
/**
 * 한국관광공사 혼잡도 및 인기 관광지 API 엔드포인트
 * TOUR_API_CONGESTION_KEY 환경 변수를 활용하여 지역별 인기 한옥 명소 10건 목록을 반환합니다.
 */
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const query = new URLSearchParams({
      ...
      MobileApp: 'OnMaru',
      _type: 'json',
      mapX: String(targetMeta.lng),
      mapY: String(targetMeta.lat),
      radius: '20000',
      contentTypeId: '12',
      arrange: 'P', // 인기순 정렬
      numOfRows: '10',
    ...
    const url = `${BASE}/locationBasedList2?serviceKey=${encodeURIComponent(apiKey)}&${query}`;
    const res = await fetch(url, { signal, next: { revalidate: 1800 } });

    if (res.ok) {
      const data = await res.json();
      const rawList = data?.response?.body?.items?.item;
      ...
      if (list.length > 0) {
        const items = list.map((item: any, idx: number) => ({
          ...
          helpfulCount: Math.max(12, 90 - idx * 7 + Math.floor(Math.random() * 5)),
          congestionLevel: idx === 0 ? '혼잡' : idx < 3 ? '보통' : '여유',
          image: toHttps(item.firstimage || item.firstimage2) || allFallbacks[idx % allFallbacks.length]?.image,
