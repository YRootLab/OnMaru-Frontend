...
interface DevCameraHelperProps {
  ...
  onToggleOrbit: (enabled: boolean) => void;
  onJumpStage: (stageIndex: number) => void;
  onUpdateTarget?: (newTarget: [number, number, number]) => void;
}
...
export default function DevCameraHelper({
  camPos,
  camTarget,
  camFov,
  activeStage,
  ...
  const handleSliderChange = (axisIndex: 0 | 1 | 2, value: number) => {
    const nextTarget: [number, number, number] = [...camTarget];
    nextTarget[axisIndex] = value;
    onUpdateTarget?.(nextTarget);
  };
  ...
  return (
    <DevContainer>
      <TitleBar>
        ...
        <ToggleButton active={isOrbitEnabled} onClick={() => onToggleOrbit(!isOrbitEnabled)}>
      ...
      <Section>
        ...
        <SliderGroup>
          <SliderRow>
            ...
            />
            <SliderValue>{camTarget[0].toFixed(1)}</SliderValue>
          </SliderRow>
          <SliderRow>
            ...
            />
            <SliderValue>{camTarget[1].toFixed(1)}</SliderValue>
          </SliderRow>
          <SliderRow>
            ...
            />
            <SliderValue>{camTarget[2].toFixed(1)}</SliderValue>
          </SliderRow>
        </SliderGroup>
      </Section>
