  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-20 right-4 z-40 inline-flex items-center gap-2 bg-[#f8f8f7] px-3.5 py-2.5 text-xs font-semibold text-[#191f28] transition-colors hover:text-[#FF2A85] sm:right-6"
      >
        <Heart size={14} strokeWidth={2} fill="currentColor" className="text-[#FF2A85]" />
        마음 담아둔 소리
        <span className="font-mono text-micro text-[#4e5968]">{savedStories.length}</span>
      </button>
        ...
        {isOpen && (
          <div className="fixed inset-0 z-50 flex justify-end" role="dialog" aria-modal="true" aria-label="마음 담아둔 소리 보관함">
            ...
            <motion.aside
              ...
              className="relative z-10 flex h-full w-full max-w-md flex-col bg-[#f8f8f7] p-6 text-[#191f28]  sm:p-8"
            >
              <div className="flex items-start justify-between   pb-5">
                <div>
                  <p className="text-xs font-semibold text-[#FF2A85]">다시 듣고 싶은 장면</p>
                  <h2 className="mt-1 font-sorimaru-sans text-2xl font-bold tracking-[-0.045em]">마음 담아둔 소리</h2>
                </div>
                <button
                  type="button"
                  onClick={() => setIsOpen(false)}
                  aria-label="보관함 닫기"
                  className="flex h-8 w-8 items-center justify-center text-xl text-[#4e5968] transition-colors hover:text-[#FF2A85]"
                >
                  <X size={22} strokeWidth={2} />
                </button>
              </div>

              <p className="mt-4 text-xs leading-5 text-[#4e5968]">
                좋아하는 이야기를 이곳에 모아두면 다음 방문에도 이어서 들을 수 있어요.
              </p>

              <div className="mt-6 flex-1 space-y-4 overflow-y-auto pr-1">
                {savedStories.length === 0 ? (
                  <div className="   py-16 text-center text-xs leading-6 text-[#4e5968]">
                    아직 담아둔 소리가 없습니다.
                    <br />
                    이야기 옆의 하트를 눌러보세요.
                  </div>
                  ...
                  savedStories.map((story, index) => {
                    ...
                    return (
                      <div key={`${story.stid}-${index}`} className="flex gap-3   pb-4">
                        <div className="h-16 w-20 shrink-0 overflow-hidden bg-[#e5e5e3]">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-micro text-[#FF2A85]">{story.locationName || '소리의 장소'}</p>
                          <button type="button" onClick={() => handlePlay(story)} className="mt-1 block max-w-full text-left">
                            <h3 className="truncate font-sorimaru-sans text-sm font-bold">{story.title}</h3>
                            <p className="mt-0.5 truncate text-micro text-[#4e5968]">{story.audioTitle}</p>
                          </button>
                          <div className="mt-2 flex items-center gap-3">
                            <button
                              type="button"
                              onClick={() => handlePlay(story)}
                              className="text-micro font-semibold text-[#FF2A85] hover:text-[#191f28]"
                            >
                            ...
                            <button
                              ...
                              className="text-micro text-[#4e5968] hover:text-[#FF2A85]"
                            >
                              담아두기 취소
                            </button>
                          </div>
              ...
              <p className="  pt-4 text-micro text-[#4e5968]">
                이 기기의 브라우저에 저장됩니다.
              </p>
            </motion.aside>
          </div>
