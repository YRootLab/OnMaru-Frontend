  ...
  <div aria-label="트랙 목록 로딩 중" className="w-full py-2" aria-busy="true">
    {Array.from({ length: 7 }, (_, index) => (
      <div key={index} className="flex items-center gap-3 border-b border-[#211e19]/[0.08] px-1 py-3.5 sm:gap-4 sm:py-4">
        <div className="sorimaru-skeleton h-3 w-5 rounded bg-[#e5e5e3]" />
        <div className="sorimaru-skeleton h-16 w-[76px] shrink-0 rounded-[10px] bg-[#d9d9d7] sm:w-20" />
        <div className="min-w-0 flex-1 space-y-2"><div className="sorimaru-skeleton h-2.5 w-20 rounded bg-[#e5e5e3]" /><div className="sorimaru-skeleton h-4 w-48 rounded bg-[#cdcdca] sm:w-72" /><div className="sorimaru-skeleton h-2.5 w-32 rounded bg-[#e5e5e3]" /></div>
        <div className="sorimaru-skeleton h-8 w-8 shrink-0 rounded-full bg-[#e5e5e3]" />
      </div>
    ))}
  </div>
);
