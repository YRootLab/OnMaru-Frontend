/*
  ...
  HanokMonthly.tsx(화면)와 scripts/build-fallback.mjs(정적 스냅샷 생성) 양쪽이 이 파일을
  그대로 import한다 — 12개월의 contentId가 스냅샷 안에 실제로 있는지는
  src/hanok/data/monthlyCurations.test.ts가 검증한다. 여기 없으면 매달 한 번씩은
  스냅샷만으로 열어도(라이브 fetch 실패 시) 글과 무관한 마을이 뜨는 사고가 난다.
*/
export const MONTHLY_CURATIONS = {
  1: {
