...
/*
  ...
  이른 봄의 이끼빛에서 시작해 한여름의 볕으로 달아올랐다가 긴 밤의 잔불빛으로 가라앉는다.

  입동·동지는 원래 서리·밤빛을 따라 파랑 계열을 썼으나, 브랜드 컬러를 주황 하나로
  묶기로 하면서(한옥 이야기 전반 동일 원칙) 이 화면에도 파랑이 섞이지 않도록
  숯불이 사위어가는 탁한 적갈색 계열로 바꿨다 — 나머지 여섯 절기와 같은 계열 안에서
  명도만 가장 낮은 자리를 맡는다.
*/
const TERM_TINTS = {
  //                밝은 바탕     어두운 바탕
  ...
  ipdong: /*  입동 */ ['#8A6355', '#B08E7F'], // 식어가는 저녁 노을
  dongji: /*  동지 */ ['#6B4A42', '#977065'], // 가장 긴 밤, 사위어가는 숯불빛
};
...
export default function SolarShadowPanel() {
  ...
  /*
    드래그(와 되돌아가기 트윈) 중에는 한 손짓 안에서 절기가 여러 번 갈린다.
    아래 텍스트들은 view.id가 바뀔 때마다 key로 리마운트되며 fadeUp을 다시 튼다 —
    끄는 동안 그 애니메이션이 절기 수만큼 반복 재생되어 번쩍이는 것으로 보인다.
    끄는 동안은 key를 얼려 리마운트를 막고, 손을 뗀 순간 한 번만 자리 잡는 페이드를 튼다.
  */
  ...
  */
  ...
  // 끄는 동안은 절기가 갈려도 같은 자리에 머문다 — 손을 뗀 뒤에야 실제 id로 자리 잡는다.
